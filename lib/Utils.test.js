...
test('offsetToColRow()', () => {
	...
	expect(
		offsetToColRow('test single line case', 10, colSize),
	).toEqual({
		row: 0,
		col: 10,
	})
...
test('isIncompleteInput()', () => {
	...
	expect(isIncompleteInput(`some 'single "double" quotes'`)).toEqual(
		false,
	)
	...
	// Incomplete quote
	expect(isIncompleteInput(`some "command that continues`)).toEqual(
		true,
	)
	expect(isIncompleteInput(`some "`)).toEqual(true)
	expect(isIncompleteInput(`some "  `)).toEqual(true)
	expect(isIncompleteInput(`some 'same thing with single`)).toEqual(
		true,
	)
	expect(isIncompleteInput(`some '`)).toEqual(true)
	expect(isIncompleteInput(`some '   `)).toEqual(true)
...
test('getSharedFragement', () => {
	...
	expect(getSharedFragment('f', ['foo-1', 'foo-2', 'a'])).toEqual(
		null,
	)
	...
	expect(getSharedFragment('f', ['foo-1', 'foo-2', 'fuu'])).toEqual(
		'f',
	)
